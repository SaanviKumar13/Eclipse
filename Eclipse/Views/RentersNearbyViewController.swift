...
class RentersNearbyViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func loadNearbyRenters() {
        db.collection("rentersNearby").getDocuments { (snapshot, error) in
            if let error = error {
            ...
            for document in snapshot!.documents {
                if let name = document.data()["name"] as? String,
                   ...
                   let profileImageURL = document.data()["profileImageURL"] as? String {
                    ...
                    let profileImage = UIImage(named: "profile") ?? UIImage()
                    
                    self.renters.append((name, distance, price, profileImage))
                    self.loadImage(from: profileImageURL) { image in
                        if let image = image, let index = self.renters.firstIndex(where: { $0.0 == name }) {
                            self.renters[index].3 = image
